...
```
EASA_Extensions_SORA v1.0.0 · cb44d23 · built 2026-09-15T14:59:54Z · 33 land use classes
```

Compare the commit and build time against what you published. If they do not
match, Pega is still serving the old bundle - republish and hard-refresh.

`build-info.json` is regenerated by the `prepublish` and `prebuildComponent` npm
hooks, so it needs no attention at publish time, and it is committed so the
build never depends on having run the script. It will therefore show as modified
after every publish. A `(dirty)` marker next to the commit means the build was
made from an uncommitted working tree.

The land use class count comes from `landusePopDensityLookup` and moves on its
own whenever those tables change.
